...
void TauolappInterface::init( const edm::EventSetup& es ){
  ...
  edm::ESHandle<HepPDT::ParticleDataTable> pdgTableHandle;
  es.getData( pdgTableHandle ) ;
  const HepPDT::ParticleDataTable* pdgTable = pdgTableHandle.product();

  init(pdgTable);
}

void TauolappInterface::init( const HepPDT::ParticleDataTable* pdgTable ){
  if ( fIsInitialized ) return; // do init only once
  if ( fPSet == 0 ) throw cms::Exception("TauolappInterfaceError") << "Attempt to initialize Tauola with an empty ParameterSet\n" << std::endl;   
...
double TauolappInterface::flat()
{
   ...
   double retVal = fRandomEngine->flat();
   //std::cout << "<TauolappInterface::flat>: retVal = " << retVal << std::endl;
   return retVal;
}
